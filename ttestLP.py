...
from math import pi
from matplotlib.pyplot import (figure,axes,plot,xlabel,ylabel,title,subplot,legend,savefig,show,xscale, xlim, clf, ylim)
...
for stime in stimes:
    ...
    for chan in chans:
        ...
        plot(1/fre1,10*np.log10(n11),'r:',label='Noise ' + st[0].stats.station + ' ' + st[0].stats.location + ' ' + \
        st[0].stats.channel )
        plot(1/fre1,10*np.log10(n22),'b:',label='Noise ' + st[1].stats.station + ' ' + st[1].stats.location + ' ' + \
        st[1].stats.channel )
        plot(1/fre1,10*np.log10(n33),'g:',label='Noise ' + st[2].stats.station + ' ' + st[2].stats.location + ' ' + \
        st[2].stats.channel )
        ...
        ns1 = 10.*np.log10(n11[(30. <= per) & (per <= 100.0)])
        ns2 = 10.*np.log10(n22[(30. <= per) & (per <= 100.0)])
        ns3 = 10.*np.log10(n33[(30. <= per) & (per <= 100.0)])

        f.write('Self-Noise 30 to 100 s sensor 1: ' + chan + ' ' + str(np.average(ns1.real)) + '+/-' + str(np.std(ns1)) + '\n')
        f.write('Self-Noise 30 to 100 s sensor 2: ' + chan + ' ' + str(np.average(ns2.real)) + '+/-' + str(np.std(ns2)) + '\n')
        f.write('Self-Noise 30 to 100 s sensor 3: ' + chan + ' ' + str(np.average(ns3.real)) + '+/-' + str(np.std(ns3)) + '\n')

        ps1 = psd1[(30. <= per) & (per <= 100.0)]
        ps2 = psd2[(30. <= per) & (per <= 100.0)]
        ps3 = psd3[(30. <= per) & (per <= 100.0)]

        f.write('PSD Noise 30 to 100 s sensor 1: ' + chan + ' ' + str(np.average(ps1.real)) + '+/-' + str(np.std(ps1)) + '\n')
        f.write('PSD Noise 30 to 100 s sensor 2: ' + chan + ' ' + str(np.average(ps2.real)) + '+/-' + str(np.std(ps2)) + '\n')
        f.write('PSD Noise 30 to 100 s sensor 3: ' + chan + ' ' + str(np.average(ps3.real)) + '+/-' + str(np.std(ps3)) + '\n')
